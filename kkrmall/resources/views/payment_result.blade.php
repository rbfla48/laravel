...
$authResultCode = $request->input('AuthResultCod');		// 인증결과 : 0000(성공)
$authResultMsg = $request->input('AuthResultMsg');		// 인증결과 메시지
$nextAppURL = $request->input('NextAppURL');				// 승인 요청 URL
$txTid = $request->input('TxTid');						// 거래 ID
$authToken = $request->input('AuthToken');				// 인증 TOKEN
$payMethod = $request->input('PayMethod');				// 결제수단
$mid = $request->input('MID');							// 상점 아이디
$moid = $request->input('Moid');							// 상점 주문번호
$amt = $request->input('Amt');							// 결제 금액
$reqReserved = $request->input('ReqReserved');			// 상점 예약필드
$netCancelURL = $request->input('NetCancelURL');			// 망취소 요청 URL
